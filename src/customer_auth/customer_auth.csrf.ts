...
export class CustomerCsrfAuthGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (!headerCsrfToken) throw new ForbiddenException("Token CSRF no proporcionado");
        ...
        if (!cachedCsrfToken || !cachedCsrfToken.csrfToken) throw new ForbiddenException("Token CSRF no encontrado o expirado");

        // Get csrf token from the cookies
        const sessionToken = request.cookies?.["iga_customer_csrf_token"];
        if (!headerCsrfToken || !sessionToken || headerCsrfToken !== sessionToken || headerCsrfToken !== cachedCsrfToken.csrfToken) throw new ForbiddenException("Token CSRF invalido");
        return true;
    };
}